...
# 03-python-operator-with-context는 provide_context=True 옵션을 주고 Atrribute에 접근
# 이 방식이 아닌 Airflow의 Template 방식을 사용. Jinja Template => Flask 자주 활용되는 템플릿
# Python에서는 Template랑 provide_context=True와 큰 차이를 못느낄 수 있지만, SQL Operator나 다른 오퍼레이션에선 유용하게 사용됨
# 쿼리문(WHERE절)에 AirFlow의 Execution_date를 인자로 넣고 실행
# Jinja Template : Airflow의 미리 정의된 템플릿. {{ ds }}, {{ ds_nodash }} 라고 정의
# Airflow Operator에 넘겨주면 실행 과정에서 템플릿 기반으로 값이 업데이트 됨
